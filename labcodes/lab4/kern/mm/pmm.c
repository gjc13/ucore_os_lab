...
page_init(void) {
    ...
    uintptr_t freemem_logical = (uintptr_t)pages + sizeof(struct Page) * npage;
    uintptr_t freemem = PADDR(freemem_logical);

    cprintf("\nphysical paging info:\n");
    cprintf("    physical page info table starts at 0x%08x(logical)\n", pages);
    cprintf("    total %d pages, page size %d bytes\n", npage, PGSIZE);
    cprintf("    free memory starts at 0x%08x\n", freemem_logical);
    ...
    for (i = 0; i < memmap->nr_map; i ++) {
        ...
        if (memmap->map[i].type == E820_ARM) {
            ...
            if (begin < end) {
                ...
                if (begin < end) {
                    cprintf("    free memory begins at page number %d\n", PPN(begin));
...
pmm_init(void) {
    ...
    cprintf("intitial pdt at %08x(physical)\n", boot_cr3);
...
get_pte(pde_t *pgdir, uintptr_t la, bool create) {
     ...
     */
    //cprintf("get pte for virtual address %08x\n", la);
    ...
    if (!(pde & PTE_P)) {            // (2) check if entry is not present
        ...
        //cprintf("new page table for pde index %u\n", page_directory_index);
        //cprintf("physical address %08x, linear address %08x\n", page_table_physical_address,
        //        page_table_linear_address);
    ...
    page_table_physical_address = (uintptr_t)PDE_ADDR(pde);
    page_table_linear_address = (uintptr_t)KADDR(PDE_ADDR(pde));
    //cprintf("found page table for pde index %u\n", page_directory_index);
    //cprintf("pde %08x\n", pde);
    //cprintf("physical address %08x, linear address %08x\n", page_table_physical_address,
    //        page_table_linear_address);
...
pgdir_alloc_page(pde_t *pgdir, uintptr_t la, uint32_t perm) {
    ...
    if (page != NULL) {
        ...
        // Lab3_X 2013011509: new allocated page doesn't have a backup in swap
        // So it is forced to be written back even if it is not dirty
        page->has_backup = 0;
        if (swap_init_ok){
            //cprintf("[pgdir_alloc_page] map swappable address 0x%08x\n", la);
