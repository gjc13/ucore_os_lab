...
alloc_proc(void) {
    ...
    if (proc != NULL) {
        ...
        proc->flags = 0;
        proc->name[0] = 0;
        //LAB5 2013011509 : (update LAB4 steps)
        /*
...
do_fork(uint32_t clone_flags, uintptr_t stack, struct trapframe *tf) {
    ...
    // LAB8:EXERCISE2 2013011509 HINT:how to copy the fs in parent's proc_struct?
...
load_icode(int fd, int argc, char **kargv) {
     ...
     */
    cprintf("Setting up new user process\n");
    if (current->mm != NULL) {
    ...
    //void * binary = kmalloc(sizeof(struct elfhdr));
    struct elfhdr *elf = kmalloc(sizeof(struct elfhdr)); 
    if (!elf) goto bad_pgdir_cleanup_mm;
    ret = load_icode_read(fd, elf, sizeof(struct elfhdr), 0);
    if (ret) goto bad_elf_cleanup_hdr;
    
    //(3.2) get the entry of the program section headers of the bianry program (ELF format)
    struct proghdr *ph = kmalloc(sizeof(struct proghdr) * elf->e_phnum); 
    if (!ph) goto bad_elf_cleanup_hdr;
    ret = load_icode_read(fd, ph, sizeof(struct proghdr) * elf->e_phnum, elf->e_phoff);
    if (ret) goto bad_elf_cleanup_ph;
    //(3.3) This program is valid?
    if (elf->e_magic != ELF_MAGIC) {
        ret = -E_INVAL_ELF;
        goto bad_elf_cleanup_ph;
    }
    ...
    //(6) setup argc and argv
    uintptr_t stack_top;
    int i;
    int total_argv_len = 0;
    for(i = 0; i < argc; i++) {
        total_argv_len += strnlen(kargv[i], EXEC_MAX_ARG_LEN + 1) + 1; // include the pending \0
    }
    stack_top = (USTACKTOP - total_argv_len - 7);
    stack_top &= ~(0x3);
    char **argv = (char **)(stack_top - argc * sizeof(char *));
    int argv_offset = 0;
    for (i = 0; i < argc; i++) {
        strcpy((char *)(stack_top + argv_offset), kargv[i]);
        argv[i] = (char *)(stack_top + argv_offset);
        argv_offset += strnlen(kargv[i], EXEC_MAX_ARG_LEN + 1) + 1;
    }
